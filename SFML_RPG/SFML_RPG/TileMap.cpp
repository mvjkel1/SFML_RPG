...
const int TileMap::getLayerSize(const int x, const int y, const int layer) const
{
	if (x >= 0 && x < static_cast<int>(this->map.size()))
	{
		if (y >= 0 && y < static_cast<int>(this->map[x].size()))
		{
			...
			{
				return this->map[x][y][this->layer].size();
			}
		}
	}
	return -1;
...
{
	/* Take three indicies (index arrays) from the mouse position in the grid and add a tile to that position, if the internal tilemap array allows it. */
...
{
	/* Take three indicies (index arrays) from the mouse position in the grid and remove a tile to that position, if the internal tilemap array allows it. */
	if (x < this->maxSizeWorldGrid.x && x >= 0 &&
		y >= 0 && y < this->maxSizeWorldGrid.y &&
		z < this->layers && z >= 0)
	{
